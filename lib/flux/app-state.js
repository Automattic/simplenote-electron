...
var actionMap = new ActionMap( {
	...
	handlers: {
		...
		tagsLoaded( state, { tags: rawTags } ) {
			// An index determines a fixed-point in the tag list,
			// but tags without indices have undefined sort, so
			// we will sort alphabetically here
			const tags = rawTags
				.slice()
				.sort( (a, b) => a.data.name.localeCompare( b.data.name ) )
				.map( (t, i) => Object.assign( t, { sortIndex: ( undefined === t.data.index ) ? t.data.index : 999999 } ) )
