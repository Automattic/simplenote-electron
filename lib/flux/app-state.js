      ...
      creator({ noteBucket, noteId, original, data, patch, isIndexing }) {
        return (dispatch, getState) => {
          ...
          if (patch && patch.content) {
            const operations = patch.content.v.split('\t').reduce((
              [ops, offset],
              next
            ) => {
              const op = next[0];
              const s = decodeURIComponent(next.slice(1));
              const length = parseInt(s, 10);

              switch (op) {
                case '+':
                  return [[...ops, [op, offset, s]], offset];
                case '-':
                  return [[...ops, [op, offset, length]], offset];
                case '=':
                  return [ops, offset + length];
              }
            }, [[], 0])[0];
          ...
          if (!diffsAreEqual) {
            ...
            const transformed = simperiumUtil.change.transform(
              working_diff,
              patch,
              original
            );
            // apply the new patch to the upstream data
            let rebased = simperiumUtil.change.apply(transformed, data);
