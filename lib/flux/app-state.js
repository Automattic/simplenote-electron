...
export const actionMap = new ActionMap({
  ...
  handlers: {
    ...
    notesLoaded(state, { notes }) {
      ...
      const isSmallScreen = window.innerWidth <= 750; // Magic number here corresponds to $single-column value in variables.scss
      // Set the selected note after loading notes
      // Selects either the first note or the previous note in the list
      if (!isSmallScreen && !state.note) {
    ...
    loadPreferences: {
      creator({ preferenceBucket }) {
        return dispatch => {
          preferenceBucket.query(db => {
            var preferences = [];
            db
              ...
              var cursor = e.target.result;
              if (cursor) {
                preferences.push(cursor.value);
                cursor.continue();
              } else {
                dispatch(this.action('preferencesLoaded', { preferences }));
              }
            };
          });
        };
    ...
    preferencesLoaded(state, { preferences }) {
      return update(state, {
        preferences: { $set: preferences },
      });
    },

    setPreference(state, { id, data, preferenceBucket }) {
      preferenceBucket.update(id, data);
    },

    setShareAnalyticsPreference: {
      creator({ enabled, preferenceBucket }) {
        return dispatch => {
          dispatch(
