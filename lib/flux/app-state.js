...
export default new ActionMap( {
	...
	handlers: {
		...
		loadNotes: {
			creator( { noteBucket } ) {
				return ( dispatch, getState ) => {
					...
					var { sortType, sortReversed } = settings;
					var sortOrder;
		...
		notesLoaded( state, { notes } ) {
			...
			return update( state, {
				notes: { $set: pinned.concat( notPinned ) }
			} );
		},
		...
		noteUpdated: {
			creator( { noteBucket, noteId, original, data, patch } ) {
				return ( dispatch, getState ) => {
					...
					let working_diff = simperium.util.change.diff( original, working );
					// generate a patch that composes both the working changes and upstream changes
					patch = simperium.util.change.transform( working_diff, patch, original );
					// apply the new patch to the upstream data
					let rebased = simperium.util.change.apply( patch, data );
