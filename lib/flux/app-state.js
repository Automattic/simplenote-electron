...
export const actionMap = new ActionMap({
  ...
  handlers: {
    ...
    notesLoaded(state, { notes }) {
      ...
      if (noteIdFromUrl) {
        selectedNote = notes.find(note => note.id === noteIdFromUrl);
      } else if (state.note) {
      ...
      if (selectedNote && selectedNote.data.deleted) {
        updatedState.showTrash = { $set: true };
      }
    ...
    setStateFromUrl(state) {
      ...
      return update(state, {
        filter: { $set: urlState.filter || null },
        note: {
          $set:
            (urlState.note && state.notes.find(note => note.id === note.id)) ||
            null,
        },
        selectedNoteId: { $set: urlState.note || null },
