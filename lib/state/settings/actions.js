...
export const setMonospaceEnabled = monospaceEnabled => ({
  type: 'setMonospaceEnabled',
  monospaceEnabled,
});

export const toggleMonospace = () => (dispatch, getState) => {
  const { settings: { monospaceEnabled } } = getState();

  dispatch(setMonospaceEnabled(!monospaceEnabled));
};
