...
describe('note view memory', () => {
  ...
  describe('caret restore on remount', () => {
    ...
    it('restores saved scroll without scrolling the caret into view', async () => {
      ...
      await flushMicrotasks();
      applyScrollRestore(shell, noteId, markdown);
      await flushMicrotasks();
  ...
  describe('getRestoreScrollTop', () => {
    it('returns saved scrollTop', () => {
      expect(getRestoreScrollTop({ scrollTop: 240 }, 'hello')).toBe(240);
    });

    it('returns zero for a missing snapshot', () => {
      expect(getRestoreScrollTop(null, 'hello')).toBe(0);
    });

    it('returns zero when saved localMarkdown differs from opening markdown', () => {
      expect(
        getRestoreScrollTop(
          {
            scrollTop: 240,
            structuredSelection: {
              anchor: { rootIndex: 0, path: [], textOffset: 0 },
              focus: { rootIndex: 0, path: [], textOffset: 0 },
              direction: 'LTR',
            },
            localMarkdown: 'old content',
          },
          'new content'
        )
      ).toBe(0);
  ...
  describe('stale localMarkdown fallback', () => {
    it('restores caret at start and scroll at top when content changed while closed', async () => {
      const noteId = 'note-stale';
      const markdown = '# title\n\nhello';

      setNoteViewState(noteId, {
        scrollTop: 200,
        structuredSelection: {
          anchor: { rootIndex: 1, path: [], textOffset: 2 },
          focus: { rootIndex: 1, path: [], textOffset: 2 },
          direction: 'LTR',
        },
        localMarkdown: 'different snapshot',
      });
