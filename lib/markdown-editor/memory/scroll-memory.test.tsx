...
describe('useNoteViewMemory scroll restore', () => {
  ...
  describe('save on unmount', () => {
    ...
    it('saves the departing note scroll on note switch, not the incoming shell', () => {
      function SwitchableNoteShell({ noteId }: { noteId: string }) {
        const shellRef = useRef<HTMLDivElement>(null);
        useNoteViewMemory({ initialMarkdown: '', shellRef, noteId });

        return (
          <div key={noteId} data-testid="shell" ref={shellRef}>
            <div data-testid="input" />
          </div>
        );
      }

      const { rerender } = render(<SwitchableNoteShell noteId="note-a" />);
      const shellA = document.querySelector(
        '[data-testid="shell"]'
      ) as HTMLElement;
      shellA.scrollTop = 420;

      rerender(<SwitchableNoteShell noteId="note-b" />);
      const shellB = document.querySelector(
        '[data-testid="shell"]'
      ) as HTMLElement;
      shellB.scrollTop = 0;

      expect(getNotePosition('note-a')).toBe(420);
    });
  });
